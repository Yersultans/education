...
const initObjects = (objectsBlocks = ObjectsDefault, Blockly) =>
  objectsBlocks.map(options => {
    ...
    Blockly.JavaScript[options.identifier] = function(block) {
      ...
      const code = value
        ? `blockFunctions.${functionName}ById && await blockFunctions.${functionName}ById('${options.identifier}', ${value});`
        : `blockFunctions.${functionName}ById && await blockFunctions.${functionName}ById('${options.identifier}', 1);`
      return code
    }
...
const jump = Blockly => {
  ...
  Blockly.JavaScript.jump = function() {
    const code =
      "EventEmitter.push({ type: 'jump', params: { identifier: 'aldar' }});"
    return code
  }
}
...
const generateAnswer = ({ key, label, Blockly }) => {
  ...
  Blockly.JavaScript[key] = function() {
    return [`\`\${askAnswers['${key}']}\``, Blockly.JavaScript.ORDER_NONE]
  }
}
...
const ask = Blockly => {
  ...
  Blockly.JavaScript.ask = function(block) {
    ...
    const askFunctionString =
      ' await new Promise((res, rej) => setTimeout(() => res(window.prompt("Введите свой ответ", "")), 3000))'
